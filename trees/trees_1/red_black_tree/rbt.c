...
int insert_rbt(rbt **root, char str[]) {
...
int main() {
    ...
    for (int i=0; i<lines; i++) {
        ...
        if (strcmp(command, "insert") == 0) {
            scanf("%s", arg);
            insert_rbt(&rbt_tree, arg);
        } else if (strcmp(command, "load") == 0) {
            scanf("%s", arg);
            if (load_rbt(&rbt_tree, arg) == 0) {
                printf("FILE NOT FOUND\n");
            }
        } else if (strcmp(command, "delete") == 0) {
            scanf("%s", arg);
            delete_rbt(&rbt_tree, arg);
        } else if (strcmp(command, "find") == 0) {
            scanf("%s", arg);
            printf("%d\n", find_rbt(rbt_tree, arg));
        } else if (strcmp(command, "min") == 0) {
            char const *min = min_rbt(rbt_tree);
            if (min != NULL) {
                printf("%s\n", min);
            } else {
            ...
            char const *max = max_rbt(rbt_tree);
            if (max != NULL) {
                printf("%s\n", max);
            } else {
                printf("Tree is empty.\n");
            ...
            scanf("%s", arg);
            rbt *successor = successor_rbt(rbt_tree, arg, true);
            if (successor != NULL) {
                printf("%s\n", successor->value);
            } else {
                printf("\n");
